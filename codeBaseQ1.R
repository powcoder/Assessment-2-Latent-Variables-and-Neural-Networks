...
#
E.step <- function(gamma, model, countss, softEM = TRUE){
  ...
  for (n in 1:N){
    ...
    if(softEM) {
      ...
      max.prob <- which(gamma[n, ]== max(gamma[n, ])) # for each point find the cluster with the maximum (estimated) probability
      gamma[n, max.prob] <- 1 # assign each point to the cluster with the highest probability
      gamma[n, !max.prob] <- 0 # remove points from clusters with lower probabilites
    }
  }
...
M.step <- function(gamma, model, counts){
  ...
  for (k in 1:K){
    model$rho[k, 1] = sum(gamma[, k])
  }
  
 # model$rho = model$rho + eps
 # model$rho = model$rho / sum(model$rho)
  
  model$rho = model$rho / N
  for (k in 1:K){
     for(w in 1:W)
     {
       model$mu[k, w] = sum(gamma[, k] * counts[w, ])
     }
    
     model$mu[k, ] = model$mu[k, ] + eps
...
EM <- function(counts, K=4, max.epoch=10, seed=123456, softEM = TRUE){
  #INPUTS:
  ...
  for(epoch in 1:max.epoch){
    ...
    # E Step
    gamma <- E.step(gamma, model, counts, softEM)
    # M Step
...
res <- EM(counts, K=4, max.epoch=5, softEM = TRUE)   
